...
def create_cnn(input_shape):
    ...
    cnn = Sequential()
    cnn.add(Conv2D(32, (3, 3), activation='elu', padding='same', kernel_regularizer=regularizers.l2(0.0001), input_shape=input_shape))
...
def main():
    ...
    hist = cnn.fit_generator(augmented_images.flow(X_train, Y_train, batch_size=64),steps_per_epoch=X_train.shape[0]/64, epochs=125, verbose=1, validation_data=(X_test, Y_test),callbacks=[LearningRateScheduler(learning_rate_scheduler), cb_early_stopping])
